...
export default function RestaurantSearchModal({
  ...
  const results = useMemo(() => {
    ...
    const byDistance = (a: { restaurant: RestaurantSummary; matchedMenu?: string }, b: { restaurant: RestaurantSummary; matchedMenu?: string }) =>
      (a.restaurant.distanceMeters ?? Infinity) - (b.restaurant.distanceMeters ?? Infinity);
    ...
    const getMatchedMenu = (r: RestaurantSummary): string | undefined => {
      if (!r.menus || !Array.isArray(r.menus)) return undefined;
      for (const m of r.menus) {
        if (!m.name) continue;
        const normName = m.name.toLowerCase().replace(/\s/g, "");
        if (normName.includes(q)) {
          return `${m.name}${m.price ? ` (${m.price})` : ""}`;
        }
        if (m.description) {
          const normDesc = m.description.toLowerCase().replace(/\s/g, "");
          if (normDesc.includes(q)) {
            return `${m.name}${m.price ? ` (${m.price})` : ""}`;
          }
        }
      }
    ...
    >
      ...
      >
        ...
        <input
          ...
          placeholder="식당 이름 또는 메뉴로 검색 (예: 돈까스, 제육)"
          autoFocus
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {!query.trim() && (
            <p className="py-6 text-center text-sm text-ink-soft">
              식당 이름이나 메뉴명(예: 삼겹살, 돈까스)으로 찾아보세요.
            </p>
          )}
          {query.trim() && results.length === 0 && (
            <p className="py-6 text-center text-sm text-ink-soft">일치하는 식당이나 메뉴가 없어요.</p>
          )}
          {results.length > 0 && (
            <ul className="flex flex-col gap-1.5">
              {results.map(({ restaurant: r, matchedMenu }) => {
                  ...
                  <li key={r.id}>
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        {matchedMenu ? (
                          <p className="truncate text-[11px] font-semibold text-primary-dark">
                            🍱 메뉴: {matchedMenu}
                          </p>
                        ) : (
